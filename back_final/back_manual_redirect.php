<?php
    ...
    if (true) {
        ...
        $stmt = $db->prepare("SELECT * FROM exam_questions as eq INNER JOIN question_bank ON eq.question_id = question_bank.qid INNER JOIN student_answers AS sa ON eqid = sa.exam_qid WHERE eq.exam_id = :eid;");
        try {
            $r = $stmt->execute([":eid" => $eid]);
            if ($r) {
